...
const NotificationService = {
    getListNotification: async (page = 0, pageSize = 5) => {
        try {
            const response = await axios.get(`${BASE_URL}/notification/3?page=${page}&size=${pageSize}`);
    ...
    seenNotification: async (id, page = 0, pageSize = 5) => {
        try {
            const response = await axios.get(`${BASE_URL}/seen-notification/${id}?page=${page}&size=${pageSize}`);
            return response.data;
        } catch (error) {
